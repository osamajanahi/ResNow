...
<body>
    ...
    <?php try {
        ...
        $firstsql="SELECT fieldid from favourite where userid=?";
        $favsql = $db->prepare($firstsql);
        $favsql->execute(array($_SESSION["activeUser"]));
        while($ids=$favsql->fetch(PDO::FETCH_ASSOC)){
            $cardids[]=$ids['fieldid'];
        }
        $secondsql = "SELECT * FROM fields WHERE id = ?";
        $fielddata = $db->prepare( $secondsql);
    } catch (PDOException $e) {
        die($e->getMessage());
    } ?>
        <section id="fields" class="sectiondownpadding">
            <main>
            <h1 class="cardcat ml-5 mb-5 ">Favorite Fields : </h1>
                <div class="container-fluid">
                    <div class="row justify-content-center cardgap mr-lg-5 ml-lg-5">
                        <?php 
                        if(isset($cardids)){
                        foreach($cardids as $id){
                            $fielddata->execute(array($id));
                            if($card=$fielddata->fetch(PDO::FETCH_ASSOC)){ ?>
...
?>
                                ...
                                <div class="card-img-top mycardcont">
                                    <img src="fieldspic\<?php echo $card[
                                        "mainpic"
                                    ]; ?>" class="card-img-top mycardimg" alt="" />
                                    <?php if (isset($_SESSION["activeUser"])) {
                                     ...
                                     <button id='favcontainer<?php echo $card[
                                         "id"
                                     ]; ?>' class="fav-button backmered" onclick="addfavorite(<?php echo $_SESSION["activeUser"] ."," .
    $card["id"]; ?>)">
                                        <i id='favicon<?php echo $card[
                                            "id"
                                        ]; ?>' class="fa-solid fa-heart-circle-check makemered"></i>
                                    </button>
                                <?php } else { ?>
                                    <button id='favcontainer<?php echo $card[
                                        "id"
                                    ]; ?>' class="fav-button" onclick="addfavorite(<?php echo $_SESSION[
    "activeUser"
] .
    "," .
    $card["id"]; ?>)">
                                ...
                                <div class="card-body mycardbody">
                                    <h5 class="card-title mycardtext mycardtitle"><?php echo $card[
                                        "name"
                                    ]; ?></h5>
                                    ...
                                    <p class="card-text mycardtext">
                                        ...
                                        Starting from: <b style="font-weight: 550"><?php echo $card[
                                            "price"
                                        ]; ?> BD</b>
                                    </p>
                        ...
                        echo " <div class=\"lonpaddingbottom\"><h1 class='cardcat'>No Favorite Fields Added</h1></div>";
                    }
                         ?>
                    </div>
                </div>
